...
import { Check, CirclePlay, LoaderCircle, Plus, RefreshCw, Settings2, Trash2, WandSparkles, X, Zap } from '../components/Icons';
...
const providerErrorMessage = (error: unknown, fallback: string) => { if (!(error instanceof Error)) return fallback; const detail = (error as Error & { detail?: string }).detail; return detail ? `${error.message}\n\nChi tiết lỗi:\n${detail}` : error.message; };
...
export function SettingsPage({ providers, onProvidersChange, settings, onSettingsChange, onNotice }: { providers: AIProvider[]; onProvidersChange: (providers: AIProvider[]) => void; settings: AppSettings; onSettingsChange: (settings: AppSettings) => void; onNotice: (message: string, kind?: 'success' | 'error') => void }) {
  ...
  const openProvider = (provider: AIProvider) => { setEditing(undefined); setProviderDraft(provider); setConfigMode('simple'); };
  ...
  return <div className="page settings-page">
    ...
    <Modal open={!!editing} title={editing?.id && providers.some((item) => item.id === editing.id) ? 'Sửa provider' : 'Thêm provider'} eyebrow="AI PROVIDER" onClose={() => setEditing(undefined)}><div className="provider-form"><label className="field"><span>Tên</span><input autoFocus value={editing?.name || ''} placeholder="My Provider" onChange={(event) => setEditing(editing ? { ...editing, name: event.target.value } : undefined)} /></label><label className="field"><span>Base URL</span><input value={editing?.baseUrl || ''} placeholder="https://example.com/v1" onChange={(event) => setEditing(editing ? { ...editing, baseUrl: event.target.value } : undefined)} /></label><label className="field"><span>API Key <small>có thể để trống</small></span><input type="password" value={editing?.apiKey || ''} placeholder="••••••••••••" onChange={(event) => setEditing(editing ? { ...editing, apiKey: event.target.value } : undefined)} /></label><label className="toggle-row"><span>Provider đang bật</span><input type="checkbox" checked={editing?.enabled ?? true} onChange={(event) => setEditing(editing ? { ...editing, enabled: event.target.checked } : undefined)} /><i /></label><div className="model-status"><span>Models</span><strong>{editing?.models.length || 0}</strong><small>{editing?.models.length ? 'Có thể chọn sau khi lưu' : 'Provider không hỗ trợ /models? Nhập Model ID thủ công ở nơi dùng.'}</small></div><button className="button ghost full" disabled={!editing || testing} onClick={() => editing && void test(editing)}><WandSparkles size={15} /> {testing ? 'Đang kết nối…' : 'Test connection & lấy models'}</button></div><div className="modal-actions"><button className="button ghost" onClick={() => setEditing(undefined)}>Hủy</button><button className="button primary" onClick={saveProvider}><Check size={15} /> Lưu provider</button></div></Modal>
  </div>;
}
