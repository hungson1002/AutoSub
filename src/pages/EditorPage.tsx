...
import { cuesToAss, cuesToSrt, downloadText, parseSubtitle, validateCues } from '../lib/subtitles';
...
export function EditorPage({ providers, settings, onSettingsChange, cues, onCuesChange, asset, onAssetChange, onNotice }: EditorProps) {
  ...
  const [seekRequest, setSeekRequest] = useState<{ id: number; timeMs: number }>();
  ...
  const [videoEdit, setVideoEdit] = useState<VideoEditState>(() => storage.videoEdit(asset?.uploadId));
  ...
  const editorMetrics = useMemo(() => {
    ...
    const original = average(enabled.map((cue) => cps(cue.originalText, cue.endMs - cue.startMs)));
    const translated = average(enabled.map((cue) => cps(cue.translatedText || cue.originalText, cue.endMs - cue.startMs)));
    const groups = (['G1', 'G2', 'G3'] as const).map((group) => {
      const groupCues = enabled.filter((cue) => cue.voiceGroup === group);
      return { group, value: average(groupCues.map((cue) => cps(cue.translatedText || cue.originalText, cue.endMs - cue.startMs))) };
    });
    const standard = groups.reduce((best, current) => Math.abs(current.value - 18) < Math.abs(best.value - 18) ? current : best, groups[0] || { group: 'G1', value: 0 });
  ...
  useEffect(() => listenForOtherDropdowns(subtitleDownloadId.current, () => setSubtitleDownloadOpen(false)), []);
  useEffect(() => {
    const close = (event: globalThis.PointerEvent) => {
      if (!subtitleDownloadRef.current?.contains(event.target as Node)) setSubtitleDownloadOpen(false);
    };
    document.addEventListener('pointerdown', close);
    return () => document.removeEventListener('pointerdown', close);
  }, []);
  useEffect(() => { assetRef.current = asset; }, [asset]);
  useEffect(() => { setVideoEdit(storage.videoEdit(asset?.uploadId)); }, [asset?.uploadId]);
  useEffect(() => { if (asset?.uploadId) storage.saveVideoEdit(asset.uploadId, videoEdit); }, [asset?.uploadId, videoEdit]);
  useEffect(() => {
      ...
      try {
        ...
        if (['completed', 'completed_with_errors', 'cancelled', 'failed'].includes(next.status) && dubbingTerminalNoticeRef.current !== `${next.id}:${next.status}`) {
          if (next.status === 'completed') {
            ...
            setDubAudioUrl(`${result.audioUrl}?v=${encodeURIComponent(next.updatedAt)}`);
            ...
            const detail = firstFailure ? ` Cue #${firstFailure.index}, bước ${firstFailure.stage}: ${friendlyErrorMessage(new Error(firstFailure.error), firstFailure.error)}` : '';
            ...
            dubbingTerminalNoticeRef.current = `${next.id}:${next.status}`;
            setDubbingJob(next);
            onNotice(next.warnings.map((warning) => friendlyErrorMessage(new Error(warning), warning)).join(' ') || 'Dubbing job thất bại.', 'error');
          }
        } else {
          setDubbingJob(next);
        }
  ...
  useEffect(() => {
    ...
    void api.getDubbingResult(dubbingJob.id).then((result) => {
      if (disposed) return;
      onCuesChange(applyDubbingMetadata(cues, result.metadata));
      setDubAudioUrl(`${result.audioUrl}?v=${encodeURIComponent(dubbingJob.updatedAt)}`);
    ...
    return () => { disposed = true; };
  }, [dubbingJob?.id, dubbingJob?.status, dubAudioUrl, cues, onCuesChange, onNotice]);
  ...
  const regenerateCueVoice = useCallback(async (cue: SubtitleCue) => {
    if (!dubbingJob || dubbingJob.status !== 'completed') {
      onNotice('Hãy tạo một dub track hoàn chỉnh trước khi tạo lại voice riêng cho cue.', 'error');
      return;
    }
    const cueIndex = cues.findIndex((item) => item.id === cue.id);
    const spokenText = applyPronunciation(cue.translatedText || cue.originalText, pronunciation).trim();
    if (!spokenText) { onNotice(`Cue #${cue.index} chưa có nội dung để đọc.`, 'error'); return; }
    ...
    try {
      ...
      setDubbingJob(next);
      onNotice(`Đang tạo lại voice riêng cho cue #${cue.index}. Các cue khác được giữ nguyên.`, 'success');
    } catch (error) {
      setRegeneratingCueId(undefined);
      onNotice(friendlyErrorMessage(error, `Không thể tạo lại voice cho cue #${cue.index}.`), 'error');
    }
  }, [cues, dubbingJob, onNotice, pronunciation]);

  const changeCue = useCallback((id: string, patch: Partial<SubtitleCue>) => onCuesChange(cues.map((cue) => {
  ...
  const reportEditorTime = useCallback((nextTimeMs: number) => { currentTimeRef.current = nextTimeMs; }, []);
  const selectCue = useCallback((id: string) => { setSelectedId(id); const cue = cues.find((item) => item.id === id); if (cue) { currentTimeRef.current = cue.startMs; setSeekRequest({ id: ++seekRequestIdRef.current, timeMs: cue.startMs }); } }, [cues]);
  ...
  const easeTranslationProgressTo = (ceiling: number) => {
    ...
    translationProgressTimerRef.current = window.setInterval(() => {
      setTranslationProgress((current) => {
        ...
        return Math.min(ceiling, current + Math.max(0.12, (ceiling - current) * 0.045));
      });
    }, 120);
  };
  ...
  const translateAll = async (setup: TranslationSetup) => {
    ...
    try {
      ...
      for (let start = 0; start < cues.length; start += batchSize) {
        ...
        const target = Math.min(94, Math.max(8, ((start + batch.length * 0.88) / cues.length) * 100));
        setTranslationStage(`Đang gửi batch ${batchNumber}/${totalBatches} · cue ${start + 1}–${start + batch.length}`);
        easeTranslationProgressTo(target);
        const result = await api.translate(provider, setup.model, batch, setup.sourceLanguage, setup.targetLanguage, setup.style, setup.customPrompt, setup.glossary.filter((entry) => entry.enabled), controller.signal);
        clearTranslationProgressTimer();
        for (const item of result.items) {
          const cue = next.find((candidate) => candidate.id === item.id);
          if (cue) cue.translatedText = item.translation;
        }
        const completed = Math.min(98, ((start + batch.length) / cues.length) * 100);
        setTranslationProgress(completed);
        setTranslationStage(`Đã nhận batch ${batchNumber}/${totalBatches} · đang lưu kết quả`);
        onCuesChange([...next]);
      }
      setTranslationProgress(100);
      setTranslationStage('Đã hoàn tất dịch toàn bộ subtitle');
      onNotice(`Đã dịch lại ${next.filter((cue) => cue.translatedText.trim()).length}/${next.length} cue.`, 'success');
    } catch (error) {
      if (error instanceof DOMException && error.name === 'AbortError') { setTranslationStage('Đã hủy dịch subtitle'); onNotice('Đã hủy xử lý trong Editor.', 'success'); }
      else { setTranslationStage('Dịch thất bại'); onNotice(friendlyErrorMessage(error, 'Dịch thất bại.'), 'error'); }
    } finally {
      clearTranslationProgressTimer();
  ...
  const runDubbing = async (configs: Record<VoiceGroup, VoiceConfig>) => {
    ...
    try {
      ...
      else onNotice(friendlyErrorMessage(error, 'Tạo dub track thất bại.'), 'error');
  ...
  const runDubbingJob = async (configs: Record<VoiceGroup, VoiceConfig>, options: DubbingRunOptions) => {
    ...
    } catch (error) { onNotice(friendlyErrorMessage(error, 'Không thể tạo dubbing job.'), 'error'); }
  };
  ...
  const downloadSubtitle = (format: 'translated' | 'original' | 'ass') => {
    ...
    if (!validation.valid) { onNotice(validation.errors[0] || 'Subtitle không hợp lệ.', 'error'); return; }
    if (format === 'ass') downloadText('autosub.ass', cuesToAss(cues, settings.subtitleStyle), 'text/x-ass');
  ...
  return <div className="page editor-page">
    ...
    <section className="editor-main"><div className="editor-left"><VideoPlayer asset={asset} cues={cues} style={settings.subtitleStyle} blurRegions={blurRegions} logo={logoOpen ? logoPreview : logo} dubAudioUrl={dubAudioUrl} audioMode={dubAudioUrl ? 'dubbed' : 'original'} dubAudioMix={dubAudioMix} seekRequest={seekRequest} onTime={reportEditorTime} onActiveCueChange={setActiveCueId} onStyleChange={styleChange} onLogoChange={previewLogoChange} onBlurRegionsChange={setBlurRegions} blurEditMode={blurEditMode} videoEdit={videoEdit} onVideoEditChange={setVideoEdit} /><div className="editor-footnote"><span><i className="status-dot" /> Autosave local</span><small>SubtitleCue[] là source of truth · {cues.filter((cue) => cue.translatedText).length}/{cues.length} bản dịch</small></div></div><div className="editor-right"><div className="editor-metrics"><div className="editor-metric"><span>TỐC ĐỘ</span><strong>Gốc: <b>{editorMetrics.original.toFixed(1)}</b> c/s · Dịch: <b>{editorMetrics.translated.toFixed(1)}</b> c/s</strong><small>TB tốc độ thoại theo cue</small></div><div className="editor-metric"><span>CPS CHUẨN</span><strong>G1: <b>{editorMetrics.groups[0]?.value.toFixed(1)}</b> · G2: <b>{editorMetrics.groups[1]?.value.toFixed(1)}</b> · G3: <b>{editorMetrics.groups[2]?.value.toFixed(1)}</b></strong><small>Chuẩn: {editorMetrics.standard.group} · {editorMetrics.standard.value.toFixed(1)} c/s</small></div></div><div className="list-heading"><div><span>SUBTITLE LIST</span><b>{cues.length}</b></div><button className="icon-button" onClick={addCue} aria-label="Thêm cue"><Plus size={16} /></button></div><SubtitleList cues={cues} activeCueId={activeCueId} selectedId={selectedId} onSelect={selectCue} onChange={changeCue} onDelete={deleteCue} onRegenerateVoice={regenerateCueVoice} regeneratingCueId={regeneratingCueId} voiceReady={dubbingJob?.status === 'completed'} /></div>{panel === 'style' && <aside className="floating-panel style-floating-panel"><div className="floating-head"><span><Settings2 size={15} /> SUBTITLE STYLE</span><button className="icon-button" onClick={() => setPanel('none')}><span aria-hidden="true">×</span></button></div><SubtitleStylePanel style={settings.subtitleStyle} onChange={styleChange} onFontUpload={setFontFile} /><div className="style-preview"><span>PREVIEW</span><div className="style-preview-surface"><span className="style-preview-text" style={{ color: settings.subtitleStyle.textColor, fontFamily: settings.subtitleStyle.fontFamily, fontSize: `${Math.max(settings.subtitleStyle.fontSize * 0.45, 12)}px`, fontWeight: settings.subtitleStyle.bold ? 700 : 400, fontStyle: settings.subtitleStyle.italic ? 'italic' : 'normal', WebkitTextFillColor: settings.subtitleStyle.textColor, WebkitTextStroke: settings.subtitleStyle.background === 'outline' ? `${(settings.subtitleStyle.outlineWidth ?? 2) * (Math.max(settings.subtitleStyle.fontSize * 0.45, 12) / Math.max(settings.subtitleStyle.fontSize, 1))}px ${settings.subtitleStyle.outlineColor}` : '0 transparent', paintOrder: 'stroke fill', background: settings.subtitleStyle.background === 'box' ? `${settings.subtitleStyle.backgroundColor ?? settings.subtitleStyle.outlineColor}${Math.round((settings.subtitleStyle.backgroundOpacity ?? 0.72) * 255).toString(16).padStart(2, '0')}` : 'transparent' }}>{selected?.translatedText || selected?.originalText || 'Bản dịch preview'}</span></div></div></aside>}</section>
    <BlurEditor open={blurOpen} regions={blurRegions} asset={asset} currentTimeMs={currentTimeRef.current} onClose={() => setBlurOpen(false)} onChange={setBlurRegions} />
