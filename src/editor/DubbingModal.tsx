...
export type DubbingRunOptions = { audioMix: { keepOriginal: boolean; originalVolume: number } };

type VoiceItem = { id: string; name?: string; language?: string };

export function DubbingModal({ open, providers, assignments, cues, pronunciation, onClose, onPronunciationChange, onRun, onNotice, job, onJobAction }: { open: boolean; providers: AIProvider[]; assignments: Record<VoiceGroup, ProviderAssignment>; cues: SubtitleCue[]; pronunciation: PronunciationEntry[]; onClose: () => void; onPronunciationChange: (entries: PronunciationEntry[]) => void; onRun: (configs: Record<VoiceGroup, VoiceConfig>, options: DubbingRunOptions) => void; onNotice?: (message: string, kind?: 'success' | 'error') => void; job?: DubbingJobStatus; onJobAction?: (action: 'pause' | 'resume' | 'cancel' | 'retry-failed') => void }) {
  ...
  const voiceItems = (currentProvider?.voices || []) as VoiceItem[];
  ...
  const playVoice = async (voiceId: string) => {
    ...
    try {
      ...
      notify(error instanceof Error ? detail ? `${error.message}\n\nChi tiết lỗi:\n${detail}` : error.message : 'Test voice thất bại.', 'error');
    }
  };
  ...
  return <Modal open={open} title="Lồng tiếng video" eyebrow="DUB WORKFLOW" onClose={onClose} wide className="dubbing-modal">
    ...
    {mode === 'voices' ? <>
      ...
      <div className="voice-config">
        ...
        <AssignmentSummary label="TTS Provider · lấy từ Cài đặt" assignment={current.assignment} provider={currentProvider} capability="tts" />
        <div className="field"><span>Voice {isElevenLabs ? '· ElevenLabs' : 'ID'}</span>
          {isElevenLabs && voiceItems.length ? <div className="voice-picker" ref={voicePickerRef}>
            <div className="voice-picker-control">
              <button type="button" className={`voice-picker-trigger ${voiceOpen ? 'active' : ''}`} onClick={() => setVoiceOpen((value) => !value)}>
            ...
            {voiceOpen && <div className="voice-picker-menu">
              ...
              <div className="voice-picker-list">{filteredVoices.length ? filteredVoices.map((voice) => <div className={`voice-picker-option ${voice.id === current.voice ? 'selected' : ''}`} key={voice.id} role="option" aria-selected={voice.id === current.voice}>
                <button type="button" className="voice-picker-option-main" onClick={() => { patchCurrent({ voice: voice.id }); setVoiceOpen(false); }}><span className="voice-picker-star">★</span><span><strong>{voice.name || voice.id}</strong><small>{voice.id}{voice.language ? ` · ${voice.language}` : ''}</small></span></button>
          ...
          </div> : <input value={current.voice} onChange={(event) => patchCurrent({ voice: event.target.value })} placeholder={isGroq ? 'Ví dụ: troy, hannah, austin' : 'Nhập Voice ID của provider'} />}
        ...
        <div className="two-fields"><div className="field"><span>Tốc độ TTS <b className="value-badge">{current.speed.toFixed(2)}x</b></span><input type="range" min={0.9} max={1.2} step={0.05} value={current.speed} onChange={(event) => patchCurrent({ speed: Number(event.target.value) })} /></div><div className="field"><span>Âm lượng <b className="value-badge">{Math.round(current.volume * 100)}%</b></span><input type="range" min={0} max={1} step={0.05} value={current.volume} onChange={(event) => patchCurrent({ volume: Number(event.target.value) })} /></div></div>
      ...
      <div className="audio-mix"><div className="section-title"><span>AUDIO MIX</span><small>Không tách vocal / nhạc nền · gốc {Math.round(originalVolume * 100)}%</small></div><label className="toggle-row"><span>Giữ âm thanh gốc</span><input type="checkbox" checked={originalVolume > 0} onChange={(event) => setOriginalVolume(event.target.checked ? 0.25 : 0)} /><i /></label><div className="field"><span>Âm lượng gốc <b className="value-badge">{Math.round(originalVolume * 100)}%</b></span><input type="range" min={0} max={1} step={0.05} value={originalVolume} onChange={(event) => setOriginalVolume(Number(event.target.value))} /></div><div className="info-note">TTS dùng bản dịch sau pronunciation dictionary, sau đó căn theo start/end bằng FFprobe + atempo.</div></div>
