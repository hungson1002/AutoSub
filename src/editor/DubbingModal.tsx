...
export type DubbingRunOptions = { slowVideoToMatchSpeech: boolean; audioMix: { mode: OriginalAudioMode; keepOriginal: boolean; originalVolume: number; separateVocals: boolean } };
...
export function DubbingModal({ open, providers, assignments, availableAssignments, cues, pronunciation, sourceVideoReady = false, sourceVideoUploading = false, onClose, onPronunciationChange, onRun, onNotice, job, onJobAction }: { open: boolean; providers: AIProvider[]; assignments: Record<VoiceGroup, ProviderAssignment>; availableAssignments: ProviderAssignment[]; cues: SubtitleCue[]; pronunciation: PronunciationEntry[]; sourceVideoReady?: boolean; sourceVideoUploading?: boolean; onClose: () => void; onPronunciationChange: (entries: PronunciationEntry[]) => void; onRun: (configs: Record<VoiceGroup, VoiceConfig>, options: DubbingRunOptions) => void; onNotice?: (message: string, kind?: 'success' | 'error') => void; job?: DubbingJobStatus; onJobAction?: (action: 'pause' | 'resume' | 'cancel' | 'retry-failed' | 'rebuild') => void }) {
  ...
  const [slowVideoToMatchSpeech, setSlowVideoToMatchSpeech] = useState(false);
  ...
  return <Modal open={open} title="Lồng tiếng video" eyebrow="DUB WORKFLOW" onClose={onClose} wide className="dubbing-modal">
    ...
    {mode === 'voices' ? <>
      ...
      <div className="voice-config">
        ...
        <label className={`dubbing-retime-option ${slowVideoToMatchSpeech ? 'active' : ''}`}>
          <input type="checkbox" checked={slowVideoToMatchSpeech} onChange={(event) => { setSlowVideoToMatchSpeech(event.target.checked); if (event.target.checked) setConfigs((value) => Object.fromEntries(groups.map((group) => [group, { ...value[group], speed: 1 }])) as Record<VoiceGroup, VoiceConfig>); }} />
          <span className="dubbing-retime-check" aria-hidden="true"><Check size={14} strokeWidth={3} /></span>
          <span className="dubbing-retime-copy"><strong>Giữ giọng 1.00x, làm chậm video theo cue</strong><small>Cue dài sẽ kéo chậm hình; video xuất có thể dài hơn bản gốc.</small></span>
          <em>VIDEO RETIME</em>
        </label>
        {slowVideoToMatchSpeech && <div className="info-note">Các cue có lời đọc dài sẽ làm chậm riêng đoạn hình tương ứng. Nếu giữ audio gốc, AutoSub cũng kéo giãn audio theo đúng các cue này mà không đổi cao độ.</div>}
        <button className="button ghost dubbing-voice-test" disabled={testing || Boolean(previewingVoice)} onClick={() => void testVoice()}><CirclePlay size={15} /> {testing ? 'Đang test…' : 'Nghe thử voice'}</button>
      </div>
      <div className="audio-mix">
        ...
        <div className="audio-component-grid" aria-label="Chọn cách dùng audio gốc">
          {([
            ...
            <input type="radio" name="source-audio-mode" checked={sourceAudioMode === modeValue} disabled={slowVideoToMatchSpeech && modeValue === 'background'} onChange={() => setSourceAudioMode(modeValue)} />
        ...
        {sourceAudioMode === 'original' && <div className="info-note warning">Audio gốc có lời thoại sẽ phát cùng giọng mới và có thể tạo tiếng vọng. Hãy chọn Tắt audio gốc nếu chỉ muốn nghe giọng lồng tiếng.</div>}
        {slowVideoToMatchSpeech && <div className="info-note">Giữ nhạc + hiệu ứng bằng Demucs chưa dùng cùng video retime. Bạn vẫn có thể giữ toàn bộ audio gốc; âm thanh sẽ được kéo giãn đúng cue.</div>}
