...
import { Crop as CropIcon, Maximize, Pause, Play, Scissors, Volume2 } from '../components/Icons';
...
type VideoPanDrag = { startX: number; startY: number; originX: number; originY: number };
...
export function VideoPlayer({ asset, cues, style = defaultStyle, blurRegions = [], logo, dubAudioUrl, audioMode = 'original', dubAudioMix, seekRequest, onTime, onActiveCueChange, roi, onRoiChange, onBlurRegionsChange, onStyleChange, onLogoChange, blurEditMode = false, videoEdit, onVideoEditChange }: Props) {
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(([entry]) => {
      if (stageResizeTimerRef.current !== undefined) window.clearTimeout(stageResizeTimerRef.current);
  ...
  const beginSubtitleDrag = (event: PointerEvent<HTMLElement>) => {
    ...
    const originY = style.position === 'top' ? 12 : style.position === 'middle' ? 50 : style.position === 'bottom' ? 82 : style.customY ?? 82;
    if (style.position !== 'custom') onStyleChange({ position: 'custom', customX: originX, customY: originY });
    event.currentTarget.setPointerCapture?.(event.pointerId);
    setSubtitleDrag({ startX: event.clientX, startY: event.clientY, originX, originY });
  };
  ...
  const zoomVideo = (event: globalThis.WheelEvent) => {
    ...
    if (!rect) return;
    const rawNextZoom = clamp(videoZoom * (event.deltaY < 0 ? 1.12 : 1 / 1.12), 0.5, 4);
    const crossesOne = (videoZoom < 1 && rawNextZoom > 1) || (videoZoom > 1 && rawNextZoom < 1);
    const nextZoom = crossesOne || Math.abs(rawNextZoom - 1) <= 0.045 ? 1 : rawNextZoom;
    if (nextZoom === 1) {
      setVideoZoom(1);
      setVideoPan({ x: 0, y: 0 });
      setAlignmentGuides({});
    ...
    setVideoPan((current) => ({ x: current.x * ratio + localX * (1 - ratio), y: current.y * ratio + localY * (1 - ratio) }));
  ...
  useEffect(() => {
    ...
    const onWheelCapture = (event: globalThis.WheelEvent) => {
      const target = event.target;
      if (!(target instanceof Node) || !workspace.contains(target) || (!event.ctrlKey && !event.metaKey)) return;
      zoomVideo(event);
    };
  ...
  const beginVideoPan = (event: PointerEvent<HTMLElement>) => {
    ...
    const drag = { startX: event.clientX, startY: event.clientY, originX: videoPan.x, originY: videoPan.y };
  ...
  const beginCanvasInteraction = (event: PointerEvent<HTMLDivElement>) => {
    ...
    const target = event.target as Element;
    if (target.closest('.subtitle-overlay, .blur-overlay, .roi-box, .logo-overlay, .video-context-menu')) return;
    beginVideoPan(event);
  };
  const showContextMenu = (event: MouseEvent<HTMLDivElement>) => {
    ...
    setContextMenu({ x: clamp(event.clientX - rect.left, 8, rect.width - 224), y: clamp(event.clientY - rect.top, 8, rect.height - 260) });
  };
  const patchVideoEdit = (patch: Partial<VideoEditState>) => {
    ...
    if (next.trimEndMs !== undefined) next.trimEndMs = Math.max(0, Math.round(next.trimEndMs));
  ...
  const beginLogoDrag = (event: PointerEvent<HTMLElement>) => {
    ...
    const rect = stageRef.current.getBoundingClientRect();
    stageRef.current.setPointerCapture?.(event.pointerId);
    const drag = { startX: event.clientX, startY: event.clientY, originX: logo.xPercent, originY: logo.yPercent, widthPercent: logo.widthPercent, stageWidth: rect.width * videoZoom, stageHeight: rect.height * videoZoom };
  ...
  const move = (event: PointerEvent<HTMLDivElement>) => {
    ...
    if (roi && onRoiChange && roiDrag) { const dx = ((event.clientX - roiDrag.startX) / (rect.width * videoZoom)) * 100; const dy = ((event.clientY - roiDrag.startY) / (rect.height * videoZoom)) * 100; onRoiChange(patchRect(roiDrag.origin, roiDrag.kind, dx, dy)); }
    if (blurDrag) {
    ...
    if (subtitleDrag) {
      const dx = ((event.clientX - subtitleDrag.startX) / (rect.width * videoZoom)) * 100;
      const dy = ((event.clientY - subtitleDrag.startY) / (rect.height * videoZoom)) * 100;
      const rawPosition = { x: clamp(subtitleDrag.originX + dx, 5, 95), y: clamp(subtitleDrag.originY + dy, 5, 95) };
      const xTargets: AlignmentGuide[] = [{ value: 50, label: 'Giữa khung' }, ...blurRegions.map((region, index) => ({ value: region.xPercent + region.widthPercent / 2, label: `Giữa blur ${index + 1}` }))];
  ...
  const stopDrag = (event?: PointerEvent<HTMLDivElement>) => {
    ...
    videoPanDragRef.current = undefined;
    setRoiDrag(undefined); setBlurDrag(undefined); setSubtitleDrag(undefined); setLogoDrag(undefined); setVideoPanDrag(undefined);
  };
  ...
  const subtitleStyle: CSSProperties = {
    ...
    background: previewStyle.background === 'box' ? `${boxColor}${Math.round(boxOpacity * 255).toString(16).padStart(2, '0')}` : 'transparent',
  ...
  const aspectRatio = effectiveVideoEdit.aspectRatio === 'original' ? '16 / 9' : effectiveVideoEdit.aspectRatio.replace(':', ' / ');
  const canvasWidth = effectiveVideoEdit.aspectRatio === '9:16' ? 'min(100%, 39.375vh)' : effectiveVideoEdit.aspectRatio === '4:5' ? 'min(100%, 56vh)' : effectiveVideoEdit.aspectRatio === '1:1' ? 'min(100%, 70vh)' : '100%';
  ...
  return <><div className="video-player">{dubAudioUrl && <audio ref={dubAudioRef} className="preview-dub-audio" src={dubAudioUrl} preload="auto" aria-hidden="true" />}<div ref={workspaceRef} className="video-workspace"><div ref={stageRef} className={`video-stage video-canvas aspect-${effectiveVideoEdit.aspectRatio.replace(':', '-')} ${blurEditMode ? 'blur-edit-stage' : ''} ${Math.abs(videoZoom - 1) >= 0.01 ? 'can-pan' : ''} ${videoPanDrag ? 'is-panning' : ''}`} style={{ aspectRatio, width: canvasWidth }} onContextMenu={showContextMenu} onPointerDown={beginCanvasInteraction} onPointerMove={move} onPointerUp={stopDrag} onPointerCancel={stopDrag}>
    <div className="video-scene-layer" style={{ transform: `translate3d(${videoPan.x}px, ${videoPan.y}px, 0) scale(${videoZoom})` }}>
    ...
    {asset && <div className="video-zoom-hud">{Math.round(videoZoom * 100)}%</div>}
    {contextMenu && <div ref={contextMenuRef} className="video-context-menu" style={{ left: contextMenu.x, top: contextMenu.y }} onPointerDown={(event) => event.stopPropagation()}><span>KHUNG HÌNH</span><div className="video-context-ratios">{(['original', '16:9', '9:16', '1:1', '4:5'] as VideoAspectRatio[]).map((ratio) => <button key={ratio} className={effectiveVideoEdit.aspectRatio === ratio ? 'active' : ''} onClick={() => { patchVideoEdit({ aspectRatio: ratio, crop: undefined }); setContextMenu(undefined); }}>{ratio === 'original' ? 'Gốc' : ratio}</button>)}</div><span>CHỈNH SỬA KHÔNG PHÁ HỦY</span><button className="video-context-action" onClick={openCrop}><CropIcon size={13} /> <span>Crop khung hình…</span>{effectiveVideoEdit.crop && <small>{effectiveVideoEdit.crop.widthPercent.toFixed(1)}% × {effectiveVideoEdit.crop.heightPercent.toFixed(1)}%</small>}</button><button className="video-context-action" onClick={openTrim}><Scissors size={13} /> <span>Cắt thời lượng…</span>{(effectiveVideoEdit.trimStartMs > 0 || effectiveVideoEdit.trimEndMs !== undefined) && <small>{formatClock(effectiveVideoEdit.trimStartMs)} – {formatClock(effectiveVideoEdit.trimEndMs ?? duration)}</small>}</button><button onClick={() => { resetVideoView(); setContextMenu(undefined); }}>Đặt lại zoom / vị trí</button></div>}
  </div></div><div className="video-controls"><button className="play-button" onClick={toggle} disabled={!asset}>{playing ? <Pause size={15} fill="currentColor" /> : <Play size={15} fill="currentColor" />}</button><span className="timecode">{formatClock(time)}</span><RangeInput className="seekbar" min={effectiveVideoEdit.trimStartMs} max={effectiveVideoEdit.trimEndMs || duration || 1} value={clamp(time, effectiveVideoEdit.trimStartMs, effectiveVideoEdit.trimEndMs || duration || 1)} onChange={(event) => seek(Number(event.target.value))} /><span className="timecode muted">{formatClock(effectiveVideoEdit.trimEndMs || duration)}</span><div className="volume-control" ref={volumeControlRef}><button className="volume-button" onClick={() => { if (!volumeOpen) announceDropdownOpen(volumeDropdownId.current); setVolumeOpen((value) => !value); }} aria-label="Điều chỉnh âm lượng"><Volume2 size={15} /></button>{volumeOpen && <div className="volume-popover"><div className="volume-popover-head"><span>Âm lượng</span><b>{Math.round(volume * 100)}%</b></div><RangeInput className="volume-popover-range" min={0} max={1} step={0.01} value={volume} onChange={(event) => setVolume(Number(event.target.value))} /></div>}</div><button className="icon-button" onClick={() => void stageRef.current?.requestFullscreen?.()} aria-label="Toàn màn hình"><Maximize size={15} /></button></div></div><VideoTrimModal open={trimOpen} asset={asset} durationMs={duration} value={effectiveVideoEdit} onClose={() => setTrimOpen(false)} onApply={(trim) => { patchVideoEdit(trim); if (time < trim.trimStartMs || (trim.trimEndMs !== undefined && time > trim.trimEndMs)) seek(trim.trimStartMs); setTrimOpen(false); }} /><VideoCropModal open={cropOpen} asset={asset} currentTimeMs={time} value={effectiveVideoEdit} onClose={() => setCropOpen(false)} onApply={(next) => { patchVideoEdit(next); resetVideoView(); setCropOpen(false); }} /></>;
}
