...
export function SubtitleStylePanel({ style, onChange, onFontUpload }: { style: SubtitleStyle; onChange: (patch: Partial<SubtitleStyle>) => void; onFontUpload?: (file: File) => void }) {
  ...
  return <div className="style-panel">
    ...
    <div className="field"><span>Cỡ chữ <b className="value-badge">{style.fontSize}px</b></span><input type="range" min={18} max={96} value={style.fontSize} onChange={(event) => onChange({ fontSize: Number(event.target.value) })} /></div>
    <div className="field"><span>Kích thước viền <b className="value-badge">{outlineWidth}px</b></span><input type="range" min={0} max={8} step={1} value={outlineWidth} onChange={(event) => onChange({ outlineWidth: Number(event.target.value) })} /></div>
