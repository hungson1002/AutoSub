...
export function LogoModal({ open, logo, externalPosition, uploadedFonts = [], onFontUpload, onClose, onChange, onPreviewChange }: { open: boolean; logo?: LogoOverlay; externalPosition?: Pick<LogoOverlay, 'position' | 'xPercent' | 'yPercent'>; uploadedFonts?: UploadedSubtitleFont[]; onFontUpload?: (file: File, family: string) => Promise<void> | void; onClose: () => void; onChange: (logo: LogoOverlay) => void; onPreviewChange?: (logo?: LogoOverlay) => void }) {
  ...
  return <Modal open={open} title="Thiết lập Logo / Watermark" eyebrow="VIDEO BRANDING" onClose={close} className="logo-modal" backdropClassName="logo-modal-backdrop">
    ...
    {draft.kind === 'text' && <div className="logo-text-options"><FontLibraryPicker value={draft.fontFamily} onChange={(fontFamily) => patch({ fontFamily })} uploadedFonts={uploadedFonts} onFontUpload={onFontUpload} label="Font chữ logo" /><label className="field"><span>Cỡ chữ <b className="value-badge">{draft.fontSize}px</b></span><RangeInput min={12} max={96} value={draft.fontSize} onChange={(event) => patch({ fontSize: Number(event.target.value) })} /></label><label className="field"><span>Màu chữ</span><input type="color" value={draft.textColor} onChange={(event) => patch({ textColor: event.target.value })} /></label><label className="field"><span>Màu viền</span><input type="color" value={draft.outlineColor} onChange={(event) => patch({ outlineColor: event.target.value })} /></label></div>}
    ...
    <div className="two-fields logo-range-row"><label className="field"><span>Độ mờ (Opacity) <b className="value-badge">{Math.round(draft.opacity * 100)}%</b></span><RangeInput min={0.1} max={1} step={0.05} value={draft.opacity} onChange={(event) => patch({ opacity: Number(event.target.value) })} /></label><label className="field"><span>Kích thước (Scale) <b className="value-badge">{Math.round((draft.widthPercent / 18) * 100)}%</b></span><RangeInput min={1.8} max={60} step={0.1} value={draft.widthPercent} onChange={(event) => patch({ widthPercent: Number(event.target.value) })} /></label></div>
    <div className="logo-modal-preview"><span>PREVIEW</span><div>{draft.kind === 'image' && draft.url ? <img src={draft.url} alt="" style={{ opacity: draft.opacity, width: `${Math.min(draft.widthPercent * 2.2, 75)}%` }} /> : <strong style={{ color: draft.textColor, WebkitTextFillColor: draft.textColor, WebkitTextStroke: `1px ${draft.outlineColor}`, paintOrder: 'stroke fill', fontFamily: draft.fontFamily, fontSize: `${Math.min(draft.fontSize, 44)}px`, textShadow: 'none' }}>{draft.text || 'AutoSub'}</strong>}</div></div>
