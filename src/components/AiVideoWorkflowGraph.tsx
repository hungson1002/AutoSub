...
export function AiVideoWorkflowGraph({ job, brief, characterFile, aspectRatio, busy, flowConnected, videoReady, canStart, setupOpen, noteNodes, providers, directorAssignments, directorAssignment, videoModel, videoModels, imageModel, imageModels, automationMode, workflowNodes, onBriefChange, onCharacterChange, onSetupToggle, onSetupClose, onNewWorkflow, onNoteNodesChange, onDirectorAssignmentChange, onVideoModelChange, onImageModelChange, onAddWorkflowNode, onRemoveWorkflowNode, onApprove, onCancel, onResume, onCompose, onSaveBible, onSaveScene, onRegenerate, onRegenerateShot, onAcceptCandidate }: Props) {
  ...
  const activeCharacterIndex = job?.status === 'designing' && !/storyboard/i.test(job.stage)
    ? Number(/nhân vật\s+(\d+)/i.exec(job.stage)?.[1] || job.characters?.find((character) => !character.sheetReady)?.index || 0)
    : 0;
  const activeStoryboardRange = /storyboard\s+(\d+)(?:\s*[–-]\s*(\d+))?/i.exec(job?.stage || '');
  const activeStoryboardStart = Number(activeStoryboardRange?.[1] || 0);
  const activeStoryboardEnd = Number(activeStoryboardRange?.[2] || activeStoryboardStart);
    ...
    <section className="ai-flow-workspace" aria-label="Canvas workflow sản xuất phim AI">
        ...
        <div className="ai-flow-stage-size" style={{ width: canvasWidth * scale, height: canvasHeight * scale }}>
          <div className="ai-flow-stage" style={{ width: canvasWidth, height: canvasHeight, transform: `scale(${scale})` }}>
            ...
            {job && nodeVisible('character') && characterRows.map((character, index) => {
              ...
              const characterRunning = character.index === activeCharacterIndex;
              return <article key={character.index} className={`ai-flow-node character ${characterRunning ? 'running' : ''} ${characterFailed ? 'error' : ''} ${character.designDirty ? 'dirty' : ''} ${selectedNode === characterNodeId ? 'selected' : ''}`} {...nodeProps(characterNodeId, { left: columns.character, top: 70 + index * 260 })} onClick={() => selectNode(characterNodeId)}>
                <header><span>CHARACTER {String(character.index).padStart(2, '0')}</span><i>{character.designDirty && !characterRunning ? '!' : character.sheetReady ? <Check size={11} /> : characterRunning ? <LoaderCircle className="spin" size={12} /> : characterFailed ? <X size={11} /> : character.index}</i></header>
                {character.sheetReady ? <button type="button" className="ai-flow-media-open" onClick={(event) => { event.stopPropagation(); setMediaPreview({ type: 'image', src: characterUrl, title: `Character bible · ${character.name}` }); }}><img src={characterUrl} alt={`Character sheet của ${character.name}`} /><Maximize size={14} /></button> : <div className={`ai-flow-node-media empty ${characterFailed ? 'error' : 'pending'}`} title={characterFailed ? job.error : undefined}>{characterRunning ? <LoaderCircle className="spin" size={22} /> : characterFailed ? <X size={25} /> : <Film size={25} />}<small>{characterFailed ? 'Tạo ảnh thất bại · bấm ↻ để thử lại' : characterRunning ? 'Đang tạo nhân vật…' : 'Đang chờ tạo nhân vật'}</small></div>}
            ...
            {sceneRows.map((scene, index) => {
              ...
              const storyboardRunning = !storyboardReady && sceneIndex >= activeStoryboardStart && sceneIndex <= activeStoryboardEnd;
              ...
              return (
                <div key={sceneIndex}>
                  {nodeVisible(`story-${sceneIndex}`) && <article className={`ai-flow-node storyboard ${storyboardReady ? 'ready' : storyboardRunning ? 'running' : ''} ${scene?.designDirty ? 'dirty' : ''} ${selectedNode === `story-${sceneIndex}` ? 'selected' : ''}`} {...nodeProps(`story-${sceneIndex}`, { left: columns.storyboard, top: y })} onClick={() => selectNode(`story-${sceneIndex}`)}>
                    <header><span>STORYBOARD FRAME {String(sceneIndex).padStart(2, '0')}</span><i>{scene?.designDirty && !storyboardRunning ? '!' : storyboardReady ? <Check size={11} /> : storyboardRunning ? <LoaderCircle className="spin" size={12} /> : sceneIndex}</i></header>
                    {storyboardReady && job && !storyboardFailed ? <button type="button" className="ai-flow-media-open" onClick={(event) => { event.stopPropagation(); setMediaPreview({ type: 'image', src: storyboardUrl, title: `Storyboard ${sceneIndex} · ${scene.title}` }); }}><img src={storyboardUrl} onError={() => setFailedMedia((items) => new Set(items).add(`story-${sceneIndex}`))} alt={`Storyboard ${sceneIndex}`} /><Maximize size={14} /></button> : <div className={`ai-flow-node-media empty ${storyboardFailed ? 'error' : 'pending'}`}>{storyboardRunning && !storyboardFailed ? <LoaderCircle className="spin" size={22} /> : storyboardFailed ? <X size={22} /> : <Film size={24} />}<small>{storyboardFailed ? 'Ảnh bị thiếu · bấm tạo lại' : storyboardRunning ? 'Đang tạo storyboard…' : 'Storyboard chưa được tạo'}</small></div>}
      ...
      {selectedNode && <aside className="ai-flow-inspector" aria-label="Chỉnh sửa node đang chọn">
        ...
        {selectedNode === 'brief' ? <div className="ai-flow-inspector-body">
          ...
          {selectedNode.startsWith('character-') && <div className="field"><span>Model tạo character sheet</span><SelectField ariaLabel="Model tạo character sheet" value={imageModel} onChange={onImageModelChange} options={imageModels.map(({ id, label }) => ({ value: id, label, description: 'Tạo ảnh nhân vật và storyboard' }))} /></div>}
          ...
          {selectedNode?.startsWith('story-') && <div className="field"><span>Model tạo storyboard</span><SelectField ariaLabel="Model tạo storyboard" value={imageModel} onChange={onImageModelChange} options={imageModels.map(({ id, label }) => ({ value: id, label, description: 'Tạo ảnh nhân vật và storyboard' }))} /></div>}
