...
import type { AIProvider, AiVideoJobStatus, AiVideoScene, FlowVideoAspectRatio, FlowVideoModel, ProviderAssignment } from '../types';
...
type Props = {
  ...
  onVideoModelChange: (value: FlowVideoModel) => void;
...
export function AiVideoWorkflowGraph({ job, brief, characterFile, aspectRatio, busy, flowConnected, canStart, setupOpen, noteNodes, providers, directorAssignments, directorAssignment, videoModel, imageModel, automationMode, workflowNodes, onBriefChange, onCharacterChange, onSetupToggle, onSetupClose, onNewWorkflow, onNoteNodesChange, onDirectorAssignmentChange, onVideoModelChange, onImageModelChange, onAddWorkflowNode, onRemoveWorkflowNode, onApprove, onCancel, onResume, onSaveBible, onSaveScene, onRegenerate, onRegenerateShot }: Props) {
    ...
    <section className="ai-flow-workspace" aria-label="Canvas workflow sản xuất phim AI">
      <header className="ai-flow-toolbar">
        ...
        <div className="ai-flow-actions">
          ...
          {job && ['failed', 'cancelled'].includes(job.status) && <button type="button" className="button secondary" disabled={busy || !flowConnected} onClick={onResume}><RotateCcw size={14} /> Tiếp tục</button>}
        ...
        <div className="ai-flow-stage-size" style={{ width: canvasWidth * scale, height: canvasHeight * scale }}>
          <div className="ai-flow-stage" style={{ width: canvasWidth, height: canvasHeight, transform: `scale(${scale})` }}>
            ...
            {job && nodeVisible('character') && characterRows.map((character, index) => {
              ...
              return <article key={character.index} className={`ai-flow-node character ${job.status === 'designing' && !character.sheetReady ? 'running' : ''} ${planningFailed ? 'error' : ''} ${character.designDirty ? 'dirty' : ''} ${selectedNode === characterNodeId ? 'selected' : ''}`} {...nodeProps(characterNodeId, { left: columns.character, top: 70 + index * 260 })} onClick={() => selectNode(characterNodeId)}>
                <header><span>CHARACTER {String(character.index).padStart(2, '0')}</span><i>{character.designDirty ? '!' : character.sheetReady ? <Check size={11} /> : job.status === 'designing' ? <LoaderCircle className="spin" size={12} /> : planningFailed ? <X size={11} /> : character.index}</i></header>
                {character.sheetReady ? <button type="button" className="ai-flow-media-open" onClick={(event) => { event.stopPropagation(); setMediaPreview({ type: 'image', src: characterUrl, title: `Character bible · ${character.name}` }); }}><img src={characterUrl} alt={`Character sheet của ${character.name}`} /><Maximize size={14} /></button> : <div className={`ai-flow-node-media empty ${planningFailed ? 'error' : 'pending'}`}>{planningFailed ? <X size={25} /> : <Film size={25} />}<small>{planningFailed ? 'Chưa tạo ảnh · kế hoạch đầu vào bị lỗi' : 'Đang chờ tạo nhân vật'}</small></div>}
                <footer><span><strong>{character.name}</strong><small>Góc nhìn · biểu cảm · trang phục</small></span><button type="button" title={character.sheetReady ? `Tạo lại ${character.name}` : `Tạo ${character.name}`} disabled={busy || job.status !== 'reviewing' || !flowConnected} onClick={(event) => { event.stopPropagation(); onRegenerate('character-sheet', undefined, character.index); }}><RotateCcw size={13} /></button></footer>
              </article>;
            })}
              ...
              return (
                <div key={sceneIndex}>
                  {nodeVisible(`story-${sceneIndex}`) && <article className={`ai-flow-node storyboard ${storyboardReady ? 'ready' : job?.status === 'designing' ? 'running' : ''} ${scene?.designDirty ? 'dirty' : ''} ${selectedNode === `story-${sceneIndex}` ? 'selected' : ''}`} {...nodeProps(`story-${sceneIndex}`, { left: columns.storyboard, top: y })} onClick={() => selectNode(`story-${sceneIndex}`)}>
                    ...
                    <footer><span><strong>{scene?.title || `Cảnh ${sceneIndex}`}</strong><small>{scene?.shotSize && scene?.lensMm ? `${scene.shotSize} · ${scene.lensMm}mm · ${scene.cameraMovement || 'locked'}` : scene?.dramaticBeat || 'Chờ AI Director'}</small></span><button type="button" title={storyboardReady ? 'Tạo lại storyboard và vô hiệu video phía sau' : 'Chạy node storyboard'} disabled={busy || job?.status !== 'reviewing' || !flowConnected || !job?.characterSheetReady} onClick={(event) => { event.stopPropagation(); onRegenerate('storyboard', sceneIndex); }}><RotateCcw size={13} /></button></footer>
                  </article>}
                  {nodeVisible(`shot-${sceneIndex}`) && <article className={`ai-flow-node shot ${scene?.status || 'pending'} ${selectedNode === `shot-${sceneIndex}` ? 'selected' : ''}`} {...nodeProps(`shot-${sceneIndex}`, { left: columns.shot, top: y })} onClick={() => selectNode(`shot-${sceneIndex}`)}>
                    <header><span>FLOW SHOT {String(sceneIndex).padStart(2, '0')}</span><i>{clipReady ? <Check size={11} /> : scene?.status === 'generating' ? <LoaderCircle className="spin" size={12} /> : sceneIndex}</i></header>
                    <div className={`ai-flow-node-media shot-preview ${!clipReady ? 'pending' : ''}`}>{clipReady && job ? <><video controls muted playsInline preload="metadata" src={aiVideoClipUrl(job.id, sceneIndex)} /><button type="button" className="ai-flow-video-expand" onClick={(event) => { event.stopPropagation(); setMediaPreview({ type: 'video', src: aiVideoClipUrl(job.id, sceneIndex), title: `Flow shot ${sceneIndex}` }); }}><Maximize size={14} /></button></> : <><Film size={24} /><small>{scene?.status === 'generating' ? 'Đang tạo video bằng Flow…' : storyboardFailed ? 'Storyboard bị thiếu' : storyboardReady ? 'Chưa tạo video · đã có frame duyệt' : 'Chờ storyboard frame'}</small></>}</div>
                    <footer><span><strong>{clipReady ? 'Video đã tạo' : scene?.status === 'generating' ? 'Đang tạo video…' : storyboardReady ? 'Sẵn sàng tạo video' : 'Chờ storyboard'}</strong><small>Google Flow · {scene.durationSeconds || job?.shotDurationSeconds || 4} giây · một góc máy</small></span><button type="button" title={clipReady ? 'Tạo lại shot và các node phía sau' : 'Chạy Flow shot'} disabled={busy || !flowConnected || !storyboardReady || !['reviewing', 'completed', 'failed'].includes(job?.status || '')} onClick={(event) => { event.stopPropagation(); onRegenerateShot(sceneIndex); }}><RotateCcw size={13} /></button></footer>
                  </article>}
                </div>
              );
            })}
            {job && nodeVisible('master') && <article className={`ai-flow-node master ${job.status === 'completed' ? 'completed' : job.status === 'composing' ? 'running' : ''} ${selectedNode === 'master' ? 'selected' : ''}`} {...nodeProps('master', { left: columns.master, top: masterY })} onClick={() => selectNode('master')}>
              ...
              {job.status === 'completed' && job.result ? <video className="ai-flow-master-video" controls muted playsInline preload="metadata" src={aiVideoUrl(job.id)} /> : <div className="ai-flow-node-process"><Film size={30} /><strong>{job.status === 'composing' ? 'Đang ghép phim' : 'Ghép & hậu kiểm'}</strong><small>{job.durationSeconds} giây · {job.scenes.length} shot</small></div>}
              {job?.status === 'completed' && job.result && <footer className="actions"><button type="button" onClick={(event) => { event.stopPropagation(); setMediaPreview({ type: 'video', src: aiVideoUrl(job.id), title: 'Master output' }); }}><Play size={12} /> Xem lớn</button><a href={aiVideoUrl(job.id, true)}><Download size={12} /> Tải MP4</a></footer>}
            </article>}
          </div>
        </div>
      </div>
      ...
      {libraryOpen && <aside className="ai-flow-library" aria-label="Thư viện node">
        ...
        <button type="button" disabled={!job || automatic || !job.scenes.some((scene) => scene.storyboardReady && !nodeVisible(`shot-${scene.index}`))} onClick={() => { const scene = job?.scenes.find((item) => item.storyboardReady && !nodeVisible(`shot-${item.index}`)); if (scene) { onAddWorkflowNode(`shot-${scene.index}`); selectNode(`shot-${scene.index}`); } }}><strong>Flow Video Shot · Video</strong><small>{!job ? 'Khả dụng sau khi có storyboard' : automatic ? 'Chế độ tự động sẽ tự thêm video shot' : 'Tạo hoặc tạo lại từng clip bằng Google Flow'}</small></button>
        <button type="button" disabled={!job || automatic || nodeVisible('master') || !job.scenes.length || !job.scenes.every((scene) => nodeVisible(`shot-${scene.index}`))} onClick={() => { onAddWorkflowNode('master'); selectNode('master'); }}><strong>Master Output · MP4</strong><small>{!job ? 'Khả dụng sau các Flow Video Shot' : automatic ? 'Chế độ tự động sẽ tự thêm master' : 'Tạo shot còn thiếu, ghép phim và hậu kiểm'}</small></button>
      ...
      {selectedNode && <aside className="ai-flow-inspector" aria-label="Chỉnh sửa node đang chọn">
        ...
        {selectedNode === 'brief' ? <div className="ai-flow-inspector-body">
          ...
          <button type="button" className="button primary full" disabled={busy || job.status !== 'reviewing' || !flowConnected} onClick={() => onRegenerate('character-sheet', undefined, selectedNode.startsWith('character-') ? Number(selectedNode.split('-')[1]) : undefined)}>{busy ? <LoaderCircle className="spin" size={14} /> : <WandSparkles size={14} />} Tạo lại nhân vật này</button>
          ...
          {selectedNode?.startsWith('shot-') && <div className="field"><span>Model tạo video</span><SelectField ariaLabel="Model tạo video Flow" value={videoModel} onChange={(value) => onVideoModelChange(value as FlowVideoModel)} options={[{ value: 'Flow Agent Auto', label: 'Flow Agent Auto', description: 'Tự chọn model Flow phù hợp cho shot' }]} /></div>}
          ...
          {selectedNode?.startsWith('story-') && <button type="button" className="button primary full" disabled={busy || job?.status !== 'reviewing' || !flowConnected} onClick={() => onRegenerate('storyboard', sceneDraft.index)}>{busy ? <LoaderCircle className="spin" size={14} /> : <WandSparkles size={14} />} {sceneDraft.storyboardReady ? 'Tạo lại storyboard này' : 'Chạy node storyboard'}</button>}
          {selectedNode?.startsWith('shot-') && <button type="button" className="button primary full" disabled={busy || !flowConnected || !sceneDraft.storyboardReady || !['reviewing', 'completed', 'failed'].includes(job?.status || '')} onClick={() => onRegenerateShot(sceneDraft.index)}>{busy ? <LoaderCircle className="spin" size={14} /> : <Play size={14} />} {sceneDraft.status === 'completed' ? 'Tạo lại Flow shot này' : 'Chạy Flow shot này'}</button>}
      ...
      {mediaPreview && <div className="ai-flow-preview-modal" role="dialog" aria-modal="true" aria-label={mediaPreview.title} onClick={() => setMediaPreview(undefined)}>
        <section onClick={(event) => event.stopPropagation()}><header><strong>{mediaPreview.title}</strong><button type="button" onClick={() => setMediaPreview(undefined)} aria-label="Đóng xem trước"><X size={16} /></button></header>{mediaPreview.type === 'video' ? <video controls autoPlay playsInline src={mediaPreview.src} /> : <img src={mediaPreview.src} alt={mediaPreview.title} />}</section>
      </div>}
    </section>
  );
}
