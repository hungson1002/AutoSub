...
function patchType(provider: AIProvider, nextType: ProviderType): AIProvider {
  ...
  return {
    ...
    baseUrl: nextType === 'groq' || nextType === 'elevenlabs' || nextType === 'whisper-local' || nextType === 'edge-tts' || nextType === 'vieneu-local' || nextType === 'kokoro-local' || nextType === 'hiiu-tts' || nextType === 'capcut-tts' ? presetBaseUrl(nextType, provider.baseUrl) : provider.baseUrl,
    ...auth,
...
export function ProviderConfigForm({ provider, mode, onModeChange, onChange, testing, loadingModels, onTestConnection, onLoadModels }: { provider: AIProvider; mode: ProviderConfigMode; onModeChange: (mode: ProviderConfigMode) => void; onChange: (provider: AIProvider) => void; testing: boolean; loadingModels: boolean; onTestConnection: () => void; onLoadModels: () => void }) {
  ...
  const isManagedLocal = resolved === 'whisper-local' || resolved === 'edge-tts' || resolved === 'vieneu-local' || resolved === 'kokoro-local';
  const localCloneBanner = resolved === 'vieneu-local' ? <div className="provider-detail-banner"><ShieldCheck size={16} /><div><strong>Clone giọng Việt chạy cục bộ</strong><small>Không API key/quota. Chỉ dùng giọng của bạn hoặc giọng đã được cho phép; runtime và model được tải một lần khi sử dụng.</small></div></div> : null;
  ...
  return <div className="provider-config-form">
    ...
    <div className="provider-form">
      ...
      <label className="field"><span>Base URL</span><input value={provider.baseUrl} disabled={isManagedLocal} placeholder={resolved === 'groq' ? 'https://api.groq.com/openai/v1' : resolved === 'elevenlabs' ? 'https://api.elevenlabs.io/v1' : 'https://example.com/v1'} onChange={(event) => updateBaseUrl(event.target.value)} /></label>
      {!isManagedLocal && <label className="field"><span>API Key <small>được lưu local trên máy này</small></span><input type="password" value={provider.apiKey || ''} placeholder="••••••••••••" onChange={(event) => update({ apiKey: event.target.value })} /></label>}
      {resolved === 'whisper-local' && <div className="provider-detail-banner"><ShieldCheck size={16} /><div><strong>Chạy hoàn toàn trên máy</strong><small>Không API key, không quota. Model chỉ tải một lần; STT chạy tuần tự bằng CPU để giữ RAM ổn định.</small></div></div>}
      {resolved === 'edge-tts' && <div className="provider-detail-banner"><ShieldCheck size={16} /><div><strong>Edge TTS online · 2 giọng Việt</strong><small>Không cần API key. Đây là endpoint consumer không chính thức, cần Internet và có thể bị Microsoft giới hạn tạm thời.</small></div></div>}
      {resolved === 'kokoro-local' && <div className="provider-detail-banner"><ShieldCheck size={16} /><div><strong>Kokoro TTS chạy cục bộ · giọng tiếng Anh</strong><small>Không API key/quota, không clone giọng. Model tải lần đầu khoảng 354 MB; runtime Python/ONNX cũng được cài riêng. Hiện có giọng Anh-Mỹ và Anh-Anh, không dùng cho lời thoại tiếng Việt.</small></div></div>}
      ...
      {mode === 'advanced' && <div className="provider-advanced-panel">
        <div className="provider-detail-banner"><ShieldCheck size={16} /><div><strong>{knownPreset ? `${resolved === 'groq' ? 'Groq' : resolved === 'elevenlabs' ? 'ElevenLabs' : resolved === 'whisper-local' ? 'Whisper Local' : resolved === 'edge-tts' ? 'Edge TTS' : resolved === 'kokoro-local' ? 'Kokoro Local' : resolved === 'hiiu-tts' ? 'HiiuTTS' : resolved === 'vieneu-local' ? 'VieNeu Local' : 'CapCut TTS'} preset đang hoạt động` : provider.providerType === 'auto' ? 'Chưa xác định preset' : 'Cấu hình tùy chỉnh'}</strong><small>{knownPreset ? 'Auth, endpoint và capability đang lấy theo chuẩn provider.' : 'Bật capability cần dùng và nhập endpoint nếu provider không theo chuẩn.'}</small></div></div>
