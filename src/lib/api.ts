import type { AIModel, AIProvider, AIVoice, BlurRegion, Capability, DubbingJobStatus, DubbingMetadata, GlossaryEntry, LogoOverlay, ReviewAspectRatio, ReviewJobStatus, SubtitleCue, SubtitleStyle, VoiceCloneProfile } from '../types';
...
export const reviewVideoUrl = (jobId: string, download = false) => `${MEDIA_BACKEND_ORIGIN}/api/review/jobs/${encodeURIComponent(jobId)}/video${download ? '?download=1' : ''}`;
...
export function friendlyErrorMessage(error: unknown, fallback = 'Đã xảy ra lỗi. Vui lòng thử lại.') {
  ...
  if (/whisper local|whisper\.cpp|autosub_whisper|edge tts|requirements-edge-tts|vieneu|kaldi-native-fbank|autosub_vieneu/i.test(raw)) return raw;
...
export const api = {
  ...
  listVieneuVoiceClones: (signal?: AbortSignal) => request<{ profiles: VoiceCloneProfile[]; voices: AIVoice[] }>('/api/voice-clones/vieneu', { signal }),
  createVieneuVoiceClone: async (name: string, file: File, consent: boolean, signal?: AbortSignal) => { const form = new FormData(); form.append('name', name); form.append('consent', String(consent)); form.append('file', file, file.name); const response = await fetch(`${MEDIA_BACKEND_ORIGIN}/api/voice-clones/vieneu`, { method: 'POST', body: form, signal }); if (!response.ok) await readError(response, 'Không thể tạo giọng clone.'); return response.json() as Promise<{ profile: VoiceCloneProfile; voice: AIVoice }>; },
  deleteVieneuVoiceClone: async (id: string) => { const response = await fetch(`${MEDIA_BACKEND_ORIGIN}/api/voice-clones/vieneu/${encodeURIComponent(id)}`, { method: 'DELETE' }); if (!response.ok && response.status !== 404) await readError(response, 'Không thể xóa giọng clone.'); },
  ...
  createReviewJob: (input: { uploadId: string; sourceLanguage: string; movieTitle?: string; characterGuide?: string; targetDurationSeconds: number; tone: string; customPrompt?: string; aspectRatio: ReviewAspectRatio; burnSubtitles: boolean; stt: { provider: AIProvider; model: string }; vision?: { provider: AIProvider; model: string }; script: { provider: AIProvider; model: string }; tts: { provider: AIProvider; model: string; voice: string; speed: number } }, signal?: AbortSignal) => request<ReviewJobStatus>('/api/review/jobs', { method: 'POST', body: JSON.stringify(input), signal }),
  getReviewJob: (id: string, signal?: AbortSignal) => request<ReviewJobStatus>(`/api/review/jobs/${encodeURIComponent(id)}`, { signal }),
  cancelReviewJob: (id: string) => request<ReviewJobStatus>(`/api/review/jobs/${encodeURIComponent(id)}/cancel`, { method: 'POST' }),
  youtubeStatus: () => request<{ connected: boolean; channelId?: string; channelTitle?: string; error?: string }>('/api/review/youtube/status'),
  connectYouTube: (clientId: string, clientSecret: string) => request<{ authUrl: string }>('/api/review/youtube/connect', { method: 'POST', body: JSON.stringify({ clientId, clientSecret }) }),
  disconnectYouTube: async () => { const response = await fetch('/api/review/youtube/connection', { method: 'DELETE' }); if (!response.ok) await readError(response, 'Không thể ngắt kết nối YouTube.'); },
  uploadReviewToYouTube: (id: string) => request<ReviewJobStatus>(`/api/review/jobs/${encodeURIComponent(id)}/youtube-upload`, { method: 'POST' }),
  refreshReviewYouTube: (id: string) => request<ReviewJobStatus>(`/api/review/jobs/${encodeURIComponent(id)}/youtube-refresh`, { method: 'POST' }),
  markReviewYouTube: (id: string, decision: 'passed' | 'claimed') => request<ReviewJobStatus>(`/api/review/jobs/${encodeURIComponent(id)}/youtube-decision`, { method: 'POST', body: JSON.stringify({ decision }) }),
};
