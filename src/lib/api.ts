...
export const api = {
  ...
  uploadMedia: async (file: File, signal?: AbortSignal) => { const form = new FormData(); form.append('file', file); const response = await fetch(`${MEDIA_BACKEND_ORIGIN}/api/uploads`, { method: 'POST', body: form, signal }); if (!response.ok) await readError(response, 'Không thể lưu file lên máy.'); return response.json() as Promise<{ uploadId: string; storedPath: string; filename: string; contentType: string; size: number }>; },
