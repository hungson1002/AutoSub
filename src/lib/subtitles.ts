...
export function cuesToAss(cues: SubtitleCue[], style: SubtitleStyle) {
  ...
  const outlineWidth = style.background === 'none' ? 0 : Math.max(0, Math.round(style.outlineWidth ?? 2));
  const marginV = style.position === 'top' ? 97 : style.position === 'bottom' ? 108 : 0;
  return `[Script Info]\nScriptType: v4.00+\nPlayResX: 1920\nPlayResY: 1080\n\n[V4+ Styles]\nFormat: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding\nStyle: Default,${style.fontFamily},${style.fontSize},${assColor(style.textColor)},${assColor(style.textColor)},${assColor(style.outlineColor)},${back},${style.bold ? -1 : 0},${style.italic ? -1 : 0},0,0,100,100,0,0,${borderStyle},${outlineWidth},0,${alignment},154,154,${marginV},1\n\n[Events]\nFormat: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text\n${lines.join('\n')}\n`;
}
