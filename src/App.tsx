...
export default function App() {
  ...
  const enableVieneuTts = () => setSettings((current) => {
    ...
    if (choices.some((item) => item.providerId === assignment.providerId && item.model === assignment.model)) return current;
    return updateCapabilityAssignments(current, 'tts', [...choices, assignment]);
  });
  return <><Layout page={page} setPage={setPage} cueCount={cues.length} providerCount={providers.length}>{page === 'pipeline' && <AutoPipelinePage providers={providers} settings={settings} cues={cues} asset={asset} onCuesChange={setCues} onAssetChange={setAsset} onOpenEditor={openEditor} onNotice={notice} />}<div className="route-keepalive" hidden={page !== 'translate'}><TranslatePage providers={providers} settings={settings} cues={cues} onCuesChange={setCues} onOpenEditor={openEditor} onNotice={notice} /></div><div className="route-keepalive" hidden={page !== 'extract'}><ExtractPage providers={providers} settings={settings} initialAsset={asset} onCuesChange={setCues} onAssetChange={setAsset} onOpenEditor={openEditor} onNotice={notice} /></div>{page === 'editor' && <EditorPage providers={providers} settings={settings} onSettingsChange={setSettings} cues={cues} onCuesChange={setCues} asset={asset} onAssetChange={setAsset} onNotice={notice} />}{page === 'review' && <ReviewPage providers={providers} settings={settings} initialAsset={asset} onAssetChange={setAsset} onNotice={notice} />}{page === 'product-ads' && <ProductAdPage providers={providers} settings={settings} onNotice={notice} />}{page === 'voice-clone' && <VoiceClonePage providers={providers} settings={settings} onVoicesChange={syncVieneuVoices} onEnableForTts={enableVieneuTts} onOpenEditor={openEditor} onNotice={notice} />}{page === 'douyin' && <DouyinPage onAssetChange={setAsset} onOpenExtract={(a) => { setAsset(a); setPage('extract'); }} onOpenReview={(a) => { setAsset(a); setPage('review'); }} onOpenEditor={(a) => { setAsset(a); setPage('editor'); }} onNotice={notice} />}{page === 'settings' && <SettingsPage providers={providers} onProvidersChange={setProviders} settings={settings} onSettingsChange={setSettings} onNotice={notice} />}</Layout>{toast && <div className={`toast ${toast.kind}`}><span className="toast-mark">{toast.kind === 'success' ? <Check size={14} /> : <X size={14} />}</span><span className="toast-message">{toast.message}</span><button className="icon-button" onClick={() => setToast(undefined)}><X size={14} /></button></div>}</>;
}
