...
export default function App() {
  ...
  useEffect(() => { storage.saveCues(cues); }, [cues]); useEffect(() => { storage.saveSettings(settings); }, [settings]); useEffect(() => { storage.saveProviders(providers); }, [providers]); useEffect(() => { if (!toast) return; const id = window.setTimeout(() => setToast(undefined), 4300); return () => window.clearTimeout(id); }, [toast]);
