...
const filters: Record<LocalMediaKind, string> = {
  ...
  media:
    "Media files|*.mp4;*.mkv;*.mov;*.webm;*.avi;*.m4v;*.mp3;*.wav;*.m4a;*.aac;*.flac;*.ogg|All files|*.*",
};
...
export async function pickLocalMediaFile(
  kind: LocalMediaKind,
  signal?: AbortSignal,
): Promise<string | undefined> {
  if (process.platform !== "win32")
    throw new Error("Mở file lớn không sao chép hiện chỉ hỗ trợ Windows.");
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once("close", (code) => {
      ...
      if (code !== 0) {
        reject(new Error(stderr.trim() || "Không thể mở hộp thoại chọn file."));
        return;
      }
      ...
      try {
        ...
        reject(new Error("Không thể đọc đường dẫn file đã chọn."));
      }
    });
  });
}
