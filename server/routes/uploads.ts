...
export const attachmentContentDisposition = (filename: string) =>
  inlineContentDisposition(filename).replace(/^inline;/, "attachment;");
...
export async function uploadRoutes(
    ...
    try {
      for await (const part of request.parts()) {
        ...
        if (part.fieldname === "file" && !stored)
          stored = await storeUpload(part.file, part.filename, part.mimetype);
        else await discardUploadStream(part.file);
      }
      if (stored && process.env.AUTOSUB_DEBUG_UPLOADS === "1")
        console.info(
          `[upload] ${JSON.stringify({ uploadId: stored.uploadId, storedPath: stored.storedPath, fileSize: stored.size })}`,
        );
      ...
      if (uploadError(error)) {
        request.log.warn(
          details,
          "Upload rejected by multipart/file-size layer",
        );
      ...
      return reply.code(500).send({
        error:
          "Không thể lưu file upload. Hãy kiểm tra dung lượng trống và thử lại.",
      });
    }
  });
  ...
  app.post("/api/uploads/import-local", async (request, reply) => {
    if (!isLocalOrigin(request.headers.origin))
      return reply.code(403).send({
        error: "Chỉ ứng dụng AutoSub trên máy này được phép mở file local.",
      });
    const body = request.body as { kind?: LocalMediaKind } | undefined;
    const kind = body?.kind;
    if (kind !== "video" && kind !== "audio" && kind !== "media")
      return reply.code(400).send({ error: "Loại file local không hợp lệ." });
    const controller = new AbortController();
    const onAborted = () => controller.abort();
    request.raw.once("aborted", onAborted);
    try {
      const sourcePath = await (
        options.pickLocalMediaFile || pickLocalMediaFile
      )(kind, controller.signal);
      ...
      if (error instanceof DOMException && error.name === "AbortError")
        return reply.code(499).send({ error: "Đã hủy chọn file." });
      return reply
        .code(error instanceof UploadReferenceError ? 404 : 500)
        .send({
          error:
            error instanceof Error ? error.message : "Không thể mở file local.",
        });
    } finally {
  ...
  app.delete("/api/uploads/:id", async (request, reply) => {
    try {
      ...
      return reply
        .code(error instanceof UploadReferenceError ? error.statusCode : 404)
        .send({ error: "Upload reference không tồn tại." });
    }
  });
    ...
    try {
      ...
      if (rangeHeader) {
        ...
        if (!match)
          return reply
            .code(416)
            .header("Content-Range", `bytes */${upload.size}`)
            .send();
        const requestedStart = match[1] ? Number(match[1]) : undefined;
        const requestedEnd = match[2] ? Number(match[2]) : undefined;
        start =
          requestedStart ?? Math.max(0, upload.size - (requestedEnd || 0));
        ...
        reply
          .code(206)
          .header("Content-Range", `bytes ${start}-${end}/${upload.size}`);
      }
      reply.header(
        "Content-Type",
      ...
      const download =
        (request.query as { download?: string } | undefined)?.download === "1";
      reply.header(
        "Content-Disposition",
        download
          ? attachmentContentDisposition(upload.filename)
          : inlineContentDisposition(upload.filename),
      );
      return reply.send(createReadStream(upload.absolutePath, { start, end }));
    } catch (error) {
      return reply
        .code(error instanceof UploadReferenceError ? error.statusCode : 404)
        .send({ error: "Video nguồn không còn tồn tại trên máy." });
    }
  });
}
