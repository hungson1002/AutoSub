...
export function buildOcrPrompt(language = 'Auto Detect', includeAllVisibleText = false) {
  ...
  if (includeAllVisibleText) {
    return `Transcribe verbatim every distinct text block visible anywhere in this video frame, including subtitles, titles, captions, labels and scene text. ${languageHint} Preserve the exact original language and script. Never translate, romanize, summarize, correct, merge, or explain the text. Return only a JSON array in visual reading order. Classify by the PURPOSE of the text, never by its screen position, font, color or size. Use {"text":"exact text","kind":"subtitle","xPercent":50,"yPercent":85} for spoken dialogue, narration captions, and sequential viewer-facing captions that change as the speech or story advances; these remain subtitles even when deliberately placed at the top, center, side, or near a speaker. Use {"text":"exact text","kind":"onscreen-text","xPercent":50,"yPercent":20} only for text belonging to the scene or interface, such as titles, chapter cards, names, labels, signs, logos, watermarks, counters and UI. A complete sentence is still a subtitle when it represents speech or sequential narration. Text visually attached to an object, sign, button or fixed graphic is onscreen-text. If uncertain, choose subtitle for changing spoken/narrated content and onscreen-text for persistent scene/UI text. Do not alternate kind for the same text block between frames. xPercent and yPercent are the visual center of that text block from 0 to 100 across the full frame. If there is no visible text, return [].`;
  }
  return `Transcribe verbatim only spoken-dialogue, narration, or sequential viewer-facing subtitle text visible in this video frame, regardless of its screen position, font, color or size. ${languageHint} Preserve the exact original language and script. Never translate, romanize, summarize, correct, or explain the text. Ignore titles, names, labels, signs, logos, watermarks, counters, scene text and UI text. Return only a JSON array where each item is {"text":"exact subtitle","kind":"subtitle","xPercent":50,"yPercent":85}. xPercent and yPercent are the visual center of the subtitle block from 0 to 100 across the full frame. If there is no subtitle, return [].`;
}
...
export async function extractionRoutes(app: FastifyInstance) {
  ...
  app.post('/api/extract/ocr', async (request, reply) => {
    ...
    try {
      ...
      const cues = groupOcrResults(results, Boolean(body.filterWatermark), Math.round(1000 / fps), roi, body.language);
